...
const flightUpdateSubData = new Map(Object.entries({ 'Flight Monitor Service': { description: 'Provides updates from a subscribed flight\n', subscribe: { summary: 'Inform about the status of a subscribed flight', message: { summary: 'Provides flight status on arrival and destination', payload: { type: 'object', properties: { user: { type: 'object', properties: { userName: { type: 'string', minimum: 1, description: 'user name', example: 'John Smith', 'x-parser-schema-id': '<anonymous-schema-3>' }, phoneNumber: { type: 'string', minimum: 5, description: 'phone number where notifications will be received.', example: '+13451235', 'x-parser-schema-id': '<anonymous-schema-4>' } }, 'x-parser-schema-id': '<anonymous-schema-2>' }, departure: { type: 'object', properties: { iataCode: { type: 'string', description: '2 to 3-character IATA carrier code', example: 'MAD', 'x-parser-schema-id': '<anonymous-schema-6>' }, scheduledDate: { type: 'string', format: 'date-time', description: 'scheduled datetime of the flight, local to the airport.', example: '2020-10-20 19:15', 'x-parser-schema-id': '<anonymous-schema-7>' }, gate: { type: 'string', description: 'departure gate', example: '2D', 'x-parser-schema-id': '<anonymous-schema-8>' }, terminal: { type: 'string', description: 'airport terminal', example: '4', 'x-parser-schema-id': '<anonymous-schema-9>' } }, 'x-parser-schema-id': '<anonymous-schema-5>' }, arrival: { type: 'object', properties: { iataCode: { type: 'string', description: '2 to 3-character IATA carrier code', example: 'MAD', 'x-parser-schema-id': '<anonymous-schema-11>' }, scheduledDate: { type: 'string', format: 'date-time', description: 'scheduled datetime of the flight, local to the airport.', example: '2020-10-20 19:15', 'x-parser-schema-id': '<anonymous-schema-12>' }, gate: { type: 'string', description: 'departure gate', example: '2D', 'x-parser-schema-id': '<anonymous-schema-13>' }, terminal: { type: 'string', description: 'airport terminal', example: '4', 'x-parser-schema-id': '<anonymous-schema-14>' } }, 'x-parser-schema-id': '<anonymous-schema-10>' } }, 'x-parser-schema-id': '<anonymous-schema-1>' }, 'x-parser-message-name': 'flightStatus' } } } }));
...
describe('appRelationDiscovery (version 2)', function () {
  ...
  before(async function () {      
    flightServiceDocs = getAsyncApiExamples(2);
  ...
  it('should return correct subscriber data for flight/update channel', function () {      
  ...
  it('should return correct publisher data for flight/update channel', function () {
    const updateChannelOutput = slugOutput.get(correctChannelUpdate);
    const updateChannelParsedOutput = parsedSlugOutput.get(correctChannelUpdate);
  ...
  it('should return correct subscriber data for flight/queue channel', function () {
  ...
  it('should return correct publisher data for flight/queue channel', function () {
    const queueChannelOutput = slugOutput.get(correctChannelQueue);
    const queueChannelParsedOutput = parsedSlugOutput.get(correctChannelQueue);
