...
describe('appRelationDiscovery (version 3)', function () {
  ...
  before(async function () {      
    flightServiceDocs = getAsyncApiExamples(3);
  ...
  describe('subscriber data for flight/update channel', function () {  
    ...
    it('should return correct subscribe channel from slug output', function () {
      assertSubscription(slugOutput, channelName, correctSubOperation);
    });
  
    it('should return correct subscribe channel from parsed slug output', function () {
      assertSubscription(parsedSlugOutput, channelName, correctSubOperation);
    });  
  });
  ...
  describe('publisher data for flight/update channel', function () {  
    ...
    it('should return correct publish channel from slug output', function () {
      assertPublication(slugOutput, channelName, correctPubOperation);
    });

    it('should return correct publish channel from parsed slug output', function () {
      assertPublication(parsedSlugOutput, channelName, correctPubOperation);
    });            
  });

  describe('subscriber data for channel flight/queue', function () {  
...
function assertPublication(output, channelName, correctPubOperation) {
  return function () {
    ...
    expect(actualPublications.address).to.be.equal(channelName);
  };
}

function assertSubscription(output, channelName, correctSubOperation) {  
  const channelOutput = output.get(channelName);
  const actualSubscriptions = channelOutput.sub.get(correctSubOperation);
  /* eslint-disable no-unused-expressions */
  expect(actualSubscriptions).to.be.not.null;
  expect(actualSubscriptions.address).to.be.equal(channelName);  
}
